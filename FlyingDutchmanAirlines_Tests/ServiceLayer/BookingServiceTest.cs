...
namespace FlyingDutchmanAirlines_Tests.ServiceLayer
{
    ...
    public class BookingServiceTest
    {
        ...
        public async Task CreateBooking_Success_CustomerNotInDatabase()
        {
            ...
            //CouldNotAddBookingToDatabaseException
        ...
        public async Task CreateBooking_Failure_RepositoryException_ArgumentException()
        {
            ...
            _mockCustomerRepository.Setup(repository =>
                repository.GetCustomerByName("Galileo Galilei")).Returns(Task.FromResult(new Customer("Galileo Galilei") { CustomerId = 0 }));
        ...
        public async Task CreateBooking_Failure_FlightNotInDatabase()
        {
            ...
            Assert.IsFalse(result);
            Assert.IsNotNull(exception);
            // aqui CouldNotAddBookingToDatabaseException
            Assert.IsInstanceOfType(exception, typeof(CouldNotAddBookingToDatabaseException));
        }
        ...
        {
            _mockBookingRepository.Setup(repository =>
                repository.CreateBooking(0, 0)).Throws(new CouldNotAddBookingToDatabaseException());
            _mockFlightRepository.Setup(repository =>
                repository.GetFlightByFlightNumber(0)).ReturnsAsync(new Flight());
            _mockCustomerRepository.Setup(repository =>
                repository.GetCustomerByName("Bill Gates")).Returns(Task.FromResult(new Customer("Bill Gates")));
            BookingService service = new BookingService(_mockBookingRepository.Object, _mockCustomerRepository.Object, _mockFlightRepository.Object);
